...
## Last-Updated: 2022-09-13T15:01:05+0200
################
...
variateinfo <- data.table(
    ...
    type=c('category', 'real', 'real', 'binary', 'binary',
           ...
           'real'), # 'binary' or 'integer' or 'real'
    min=c(1, 0, 12, 0, 0,
          #50, 50, 50, 50, 50, 50, 50,
          50 ), # 'binary' should have 0, 'category' 1
    max=c(8, 130, 100, 1, 0,
          #100, 100, 100, 100, 100, 100, 100,
          100 ), # 'binary' should have 1
    mean_mean=c(NA, 40, 30, NA, NA,
                #75, 75, 75, 75, 75, 75, 75,
                75), # only for 'real' variates, NA for others
    ...
    sigma_shape=c(NA, 1, 1, NA, NA,
                  #1, 1, 1, 1, 1, 1, 1,
                  1 )/2 # with 1/4, then min SD value is ~ 0.3 times the one in sigma_sqrtrate
)
...
initsFunction <- function(){
    ...
    if(nrcovs>0){# real variates
        list(
            ...
            meanRtau0=diag(1/variateinfo[variate %in% realCovs, mean_sigma]^2), # dims = inv. variance
            tauRrate0=sqrt(2)*diag(variateinfo[variate %in% realCovs, sigma_sqrtrate]), # dims = variance
            tauRshape0=variateinfo[variate %in% realCovs, sigma_shape][1]*2 + nrcovs -1
        )},
...
bayesnet <- nimbleCode({
    ...
    for(acluster in 1:nClusters){
        if(nrcovs>0){# real variates
                meanR[1:nRcovs,acluster] ~ dmnorm(mean=meanRmean0[1:nRcovs], prec=meanRtau0[1:nRcovs,1:nRcovs])
                tauR[1:nRcovs,1:nRcovs,acluster] ~ dwish(cholesky=tauRrate0[1:nRcovs,1:nRcovs], df=tauRshape0, scale_param=0)
        }
        if(nicovs>0){# integer variates
    ...
    if(posterior){# cluster occupations
        ...
        for(adatum in 1:nData){
            if(nrcovs>0){
                    Real[adatum,1:nRcovs] ~ dmnorm(mean=meanR[1:nRcovs,C[adatum]], prec=tauR[1:nRcovs,1:nRcovs,C[adatum]])
            }
            if(nicovs>0){
                for(avar in 1:nIcovs){
...
if(posterior){
    model <- nimbleModel(code=bayesnet, name='model1', constants=constants, inits=initsFunction(), data=dat, dimensions=list(q=nclusters, meanR=c(nrcovs,nclusters), tauR=c(nrcovs,nrcovs,nclusters), probI=c(nicovs,nclusters), probC=c(nccovs,nclusters,ncategories), probB=c(nbcovs,nclusters), C=ndata) )
}else{
    model <- nimbleModel(code=bayesnet, name='model1', constants=constants, inits=initsFunction(), data=list(), dimensions=list(q=nclusters, meanR=c(nrcovs,nclusters), tauR=c(nrcovs,nrcovs,clusters), probI=c(nicovs,nclusters), probC=c(nccovs,nclusters,ncategories), probB=c(nbcovs,nclusters)))
}
...
if(posterior){# Samplers for posterior sampling
    ...
    for(acluster in 1:nclusters){
        if(nrcovs>0){
                confmodel$addSampler(target=paste0('meanR[1:', nrcovs, ', ', acluster, ']'), type='conjugate')
                confmodel$addSampler(target=paste0('tauR[1:', nrcovs, ', 1:', nrcovs, ', ', acluster, ']'), type='conjugate')
        }
        if(nicovs>0){
            for(avar in 1:nicovs){
...
for(stage in stagestart+(0:nstages)){
    ...
    version <- paste0(baseversion,'_', stage)
    gc()
    ...
    saveRDS(mcsamples,file=paste0(dirname,'/_mcsamples-R',version,'-V',length(covNames),'-D',ndata,'-K',nclusters,'-I',nrow(mcsamples),'--',mcmcseed,'.rds'))
    ##
    ...
    saveRDS(finalstate2list(finalstate),file=paste0(dirname,'/_finalstate-R',version,'-V',length(covNames),'-D',ndata,'-K',nclusters,'-I',nrow(mcsamples),'--',mcmcseed,'.rds'))
    ##
    ## SAVE THE PARAMETERS FOR THE TRANSDUCER
    parmList <- mcsamples2parmlist(mcsamples, realCovs, integerCovs, categoryCovs, binaryCovs)
    saveRDS(parmList,file=paste0(dirname,'/_frequencies-R',version,'-V',length(covNames),'-D',ndata,'-K',nclusters,'-I',nrow(parmList$q),'--',mcmcseed,'.rds'))
    ##
    ## Diagnostics
    ## Log-likelihood
    ...
    saveRDS(traces,file=paste0(dirname,'/_probtraces-R',version,'-V',length(covNames),'-D',ndata,'-K',nclusters,'-I',nrow(parmList$q),'--',mcmcseed,'.rds'))
    ##
    if(nrow(traces)>=1000){
    ...
    pdff(paste0(dirname,'/mcsummary2-R',version,'-V',length(covNames),'-D',ndata,'-K',nclusters,'-I',nrow(parmList$q),'--',mcmcseed),'a4')
    ...
    for(avar in covNames){
        ...
        tplot(x=histo$breaks, y=histo$density, col=yellow, lty=1, lwd=1, xlab=avar, ylab='probability density', ylim=c(0, ymax), family=family, xlim=range(Xgrid))
        tplot(x=Xgrid, y=plotsamples, type='l', col=paste0(palette()[7], '44'), lty=1, lwd=2, xlab=avar, ylab='probability density', ylim=c(0, ymax), family=family, add=T)
    }
    ##
    par(mfrow=c(1,1))
