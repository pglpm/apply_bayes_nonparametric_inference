...
%% Last-Updated: 2023-02-06T15:20:06+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\section{Mathematical representation of the density $\p(\di F \| I)$}
\label{sec:repr_density_F}

In order to specify the probability density $\p(\di F \| I)$ we need a mathematical representation of the possible joint distributions $F(y,x)$ and of their space; from a differential-geometric point of view \autocites[III.A, VII.A]{choquetbruhatetal1977_r1996}, we want a coordinate system on the manifold of distributions $\set{F}$. It would be an advantage if this mathematical representation would also let us easily compute conditional and marginal distributions such as $F(y\| x)$ and $F(x)$, because we need these to calculate our main probability from formulae~\eqref{eq:general_prob_partialexch} or \eqref{eq:indepYfromdata}.
...
\end{multline}
where the $w$ are normalized weights and the $\upsilon$ and $\xi$ are parameters, possibly multidimensional. Any $F$ is thus represented by an in-principle infinite set of coordinates:
\begin{equation}
  \label{eq:F_coords}
  F \corr (w^{c}, \upsilon^{c}_{j}, \xi^{c}_{j}) \ .
\end{equation}
More precisely, the differential manifold parametrized by these coordinates covers the manifold $\set{F}$: different coordinate values may correspond to the same $F$. We can also say that we are expressing $F$ through a frame, rather than through a set of basis functions \autocites{balanetal2006}[\chap~8]{heil1998_r2011}[\chap~3]{daubechies1992_r1999}.
