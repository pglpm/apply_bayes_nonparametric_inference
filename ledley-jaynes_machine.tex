...
%% Last-Updated: 2023-02-06T17:18:06+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\subsection{Representation of $F$}
\label{sec:repr_F}
...
\end{multline}
where the $w$ are normalized weights and the $\upsilon$ and $\xi$ are parameters, possibly multidimensional. Any $F$ is thus represented by an in-principle infinite tuple of variables:
\begin{equation}
  \label{eq:F_coords}
...
The representation~\eqref{eq:F_repres} can in turn be embedded in a larger space; this may be convenient for computing the posterior density $\p(\di F \| D,I)$ of \eqn~\eqref{eq:common_density}. Consider a distribution $F$ represented by variables $(w, \upsilon, \xi)$, and the product of its values at specific variate values $y^{1},y^{2},\dotsc,x^{1},x^{2},\dotsc$. We may write this as
\begin{multline}
  \label{eq:product_F}
...
\dotsm \bigr] \ .
\end{multline}
In the last term we have as many sums and running indices $c^{i}$ as units $i=1,2,\dotsc$, and corresponding terms within the product. This last expression can be interpreted as the marginalization over variables $c^{1}, c^{2}, \dotsc$ of a distribution $G$ defined as
\begin{multline}
  \label{eq:distr_with_c}
  G(c^{1},y^{1},x^{1},\ c^{2},y^{2},x^{2},\dotsc \| w,\upsilon,\xi) \defd{}\\
  w^{c^{1}}\,
  K(y^{1} \| \upsilon^{c^{1}})\, K(x^{1} \| \xi^{c^{1}})\ 
  w^{c^{2}}\,
  K(y^{2} \| \upsilon^{c^{2}})\, K(x^{2} \| \xi^{c^{2}})\ 
...
\subsection{Representation of the densities}
\label{sec:repr_densities}

A probability density $\p(\di F)$ is specified by specifying a density $\p(\di w, \di\upsilon, \di\xi)$. This specification is determined partly by the information underlying the density, partly by computational demands. These densities can be calculated by a variety of Monte Carlo methods, each of which may be more convenient in a particular 

\medskip

The fact that the $(w,\upsilon,\xi)$-manifold is a covering of the manifold $\set{F}$ is not a problem, because a probability density on the first also defines a probability density on the second. Indeed what is now left is the specification of a probability density
\begin{equation}
