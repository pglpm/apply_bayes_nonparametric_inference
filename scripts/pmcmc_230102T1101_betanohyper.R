...
baseversion <- '_test_betanohyper' # Base name of output directory
...
initsFunction <- function(){
    ...
    if(len$R > 0){
        ...
        Rrate0 <- rep(rrate0, len$R)
        sRmean <- matrix(rnorm(n=nclusters*len$R, mean=Rmean0, sd=sqrt(Rvar0)),
                         nrow=len$R, ncol=nclusters)
        sRrate <- matrix(rinvgamma(n=nclusters*len$R, shape=Rshapein0, rate=Rrate0),
                         nrow=len$R, ncol=nclusters)
    ...
    for(k in 1:nclusters){
        if(len$R > 0){# real variates
            for(v in 1:Rn){
                ...
                Rrate[v, k] ~ dinvgamma(shape=Rshapein0[v], rate=Rrate0[v])
        ...
        if(len$D > 0){# censored variates
            for(v in 1:Dn){
                ...
                Drate[v, k] ~ dinvgamma(shape=Dshapein0[v], rate=Drate0[v])
        ...
        if(len$I > 0){# integer variates
            for(v in 1:In){
                ...
                Irate[v, k] ~ dinvgamma(shape=Ishapein0[v], rate=Irate0[v])
