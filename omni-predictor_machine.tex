...
%% Last-Updated: 2023-02-16T15:28:27+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\begin{equation}
  ...
  \exp\biggl[-\frac{(x-\mu)^{2}}{\sigma^{2}}\biggr]\, \di x \ .
\end{equation}
...
\begin{equation}
  \label{eq:K_cat}
  K(x \| q) \defd \sum_{i=1}^{n} q_{i}\,\delt(x=a_{i}) \equiv
  \begin{cases}
...
We can represent such a variate as an index for different subranges of an underlying continuous variate. Thus for its kernel we choose the convolution of a gaussian with mean and variance parameters $(\mu,\sigma^{2})$ and a \enquote{window function}:
\begin{equation}
  \label{eq:K_int}
  \begin{split}
  K(x \| \mu, \sigma^{2}) &\defd
  ...
  \exp\biggl[-\frac{(z-\mu)^{2}}{\sigma^{2}}\biggr]\, \di z \ .
\end{split}
\end{equation}
