...
%% Last-Updated: 2023-02-17T15:16:07+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
Variates with connected, open domains in the real line can transformed into this kind of variate. For example  a variate $z \in \opop{0, +\infty}$ can be transformed into $x=\ln z$, and a variate $z \in \opop{a,b}$  into $x=\Phi^{-1}\bigl(\frac{z-a}{b-a}\bigr)$.
...
For such a variate we can used a gaussian distribution with mean and variance parameters $(\mu,\sigma^{2})$, where the $q$-tails, for some $0<q<1/2$, represent the probability mass given to the boundary points.

For a doubly-bounded variate $x \in \clcl{a,b}$:
\begin{equation}
  \label{eq:K_cens}
    K(\di x \| \mu, \sigma^{2}) \defd
    \begin{dcases}
        \Phi\biggl[
  \frac{\Phi^{-1}(q) - \mu}{\sigma}
  \biggr]
  &\text{if $x=a$}
      \\[2\jot]
        \Phi'\biggl(
  \frac{\Phi^{-1}\bigl[(1-2\,q)\,\frac{x-a}{b-a}+q\bigr] - \mu}{\sigma}
  \biggr)
  \, \di x
  &\text{if $a < x < b$}
      \\[2\jot]
        \Phi\biggl[
  \frac{\Phi^{-1}(1-q) - \mu}{\sigma}
  \biggr]
  &\text{if $x=b$}
    \end{dcases}
\end{equation}
