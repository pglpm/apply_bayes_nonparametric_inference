...
%% Last-Updated: 2023-02-19T16:58:26+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\paragraph{Continuous unbounded variates}

For a continuous unbounded variate $x \in \RR$ we choose a gaussian distribution with mean and variance as the parameters $\xi=(\mu, \sigma^{2})$:
\begin{equation}
  \label{eq:K_unb_cont}
  \begin{split}
    &x \in \RR:
...
Other continuous variates with connected, open or semi-open domains in the real line can transformed into the continuous unbounded or censored ones discussed last with appropriate transformations. For example a variate $z \in \opop{0, +\infty}$ can be transformed into $x=\ln z$, a variate $z \in \opop{a,b}$ into $x=\Phi^{-1}\bigl(\frac{z-a}{b-a}\bigr)$, and a variate $z \in \clop{0, +\infty}$ into $x=\ln(z+a)$ for some $a>0$, to be treated as a censoring value. These kinds of transformation are often suggested by the nature of the original variate.
