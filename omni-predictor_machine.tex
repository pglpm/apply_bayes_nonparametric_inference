...
%% Last-Updated: 2023-02-28T13:34:43+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\subsection{Categorical variates}
\label{sec:variates_categorical}

For a categorical variate with $n$ possible values, $x \in \set{a_{1}, \dotsc, a_{n}}$, we choose a categorical distribution with normalized weight parameters $q \defd (q_{a_1}, \dotsc, q_{a_n})$:
\begin{equation}
  \label{eq:K_cat}
  \begin{split}
...
\subsection{Integer or discrete ordinal variates}
...
\subsection{Density for the parameters of the categorical components}
...
\begin{multline}
  ...
  \sum_{c^u}
  w^{c^u}\times{}\\
  K(x_{\vr}^u\|\mu_{\vr}^{c^u},v_{\vr}^{c^u})\,
  K(x_{\vd}^u\|\mu_{\vd}^{c^u},v_{\vd}^{c^u})\,
  K(x_{\vi}^u\|\mu_{\vi}^{c^u},v_{\vi}^{c^u})\,
  K(x_{\vb}^u\|q_{\vb}^{c^u})\,
  K(x_{\vc}^u\|q_{\vc}^{c^u})
  \biggr]
  \times{}
