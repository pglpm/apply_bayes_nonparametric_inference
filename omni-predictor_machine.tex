...
%% Last-Updated: 2023-02-26T18:37:31+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
For such a variate we can use a \enquote{discretized-Gaussian} distribution with mean and variance parameters $(\mu,\sigma^{2})$. We introduce first a function
\begin{multline}
  \label{eq:Q_function_integer}
  Q\colon \set{1,\dotsc, m} \to \RR
  \\ \text{satisfying the symmetry}\quad 
  Q(m+1-x)=Q(x) \ ,
\end{multline}
which will be described more in detail in \sect\mynotew{***}. The distribution for $x$ is then given by
\begin{equation}
  \label{eq:K_int}
  \begin{split}
    &x \in \set{1,\dotsc, m}:
...
\subsubsection{Censored continuous variates}
