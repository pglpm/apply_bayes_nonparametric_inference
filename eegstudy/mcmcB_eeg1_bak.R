...
## Last-Updated: 2022-10-25T14:40:42+0200
################
...
yxsamples <- samplesXmc(mcsamples=mcsamples, variateinfo=variateparameters, drawspermcsample=1024, fromsamples=64)
outnames <- colnames(yxsamples)
dim(yxsamples) <- c(64,1024,nvars)
dimnames(yxsamples) <- list(NULL, NULL, outnames)

testvars <- rev(c(sample(mainvar,1), sample(setdiff(varNames,mainvar),1)))
graphics.off()
pdff('test2dsamples')
par(mfrow=c(8,8),mar = c(0,0,0,0))
for(i in 1:64){
    xlim <- range(c(quantile(yxsamples[i,,testvars[1]],c(0.025,0.975)),range(alldata[[testvars[1]]])))
    ylim <- range(c(quantile(yxsamples[i,,testvars[2]],c(0.025,0.975)),range(alldata[[testvars[2]]])))
    tplot(x=list(yxsamples[i,,testvars[1]],alldata[[testvars[1]]]),
          y=list(yxsamples[i,,testvars[2]],alldata[[testvars[2]]]),
          type='p',pch=list('.',16),cex=c(1,0.5),alpha=c(0.5,0.25),
          xlim=xlim, ylim=ylim,
          xticks=NA, yticks=NA, mar=c(1,1,1,1),
          xlab=(if(i==1){testvars[1]}),
          ylab=(if(i==1){testvars[2]})
          )
}
dev.off()
