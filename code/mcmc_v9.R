...
## Last-Updated: 2022-06-25T18:05:57+0200
################
...
if(availableCores()>1){
    if(.Platform$OS.type=='unix'){
        plan(multicore, workers=max(availableCores()-1,1))
    }else{
        plan(multisession, workers=max(availableCores()-1,1))
    }
}else{
    plan(sequential)
}
...
baseversion <- '_TEST'
nclusters <- 64L
nsamples <- 1024L * 1L # number of samples AFTER thinning
niter0 <- 1024L * 2L # iterations burn-in
thin <- 1L # thinning
nstages <- 1L # number of sampling stages beyond burn-in
maincov <- 'BINNAME'
family <- 'Palatino'
## ndata <- *** # set this if you want to use fewer data
...
variateinfo <- data.table(
    variate=c('BINNAME', 'INTNAME', 'REANAME'),
    type=c('binary', 'integer', 'real'), # 'binary' or 'integer' or 'real'
    min=c(0, 0, -100), # 'binary' should have 0
    max=c(1, 9, 100), # 'binary' should have 1
