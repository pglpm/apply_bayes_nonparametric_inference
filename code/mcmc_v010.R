...
##
fwrite(cbind(data.table(variate=varNames),variatepars), file=paste0(dirname,'/variateparameters.csv'))
...
initsFunction <- function(){
    ...
    if(compoundgamma & nrvars > 0){
        list(varRshape1 = variatepars[realVars,'shape1rate'])
    }else{
        list(varRrate = variatepars[realVars,'shape1rate'])
    },
    if(nivars > 0){# integer variates
        list(
            probIa0 = rep(2, nivars),
            probIb0 = rep(1, nivars),
            sizeIprob0 = matrixprobivars*matrixprobivars,
            sizeI = matrix(maxivars, nrow=nivars, ncol=nclusters)
        )},
...
if(posterior){
    model <- nimbleModel(code=bayesnet, name='model1', constants=constants, inits=initsFunction(), data=dat, dimensions=c(list(q=nclusters, meanR=c(nrvars,nclusters), tauR=c(nrvars,nclusters), probI=c(nivars,nclusters), probC=c(ncvars,nclusters,ncategories), probB=c(nbvars,nclusters), C=ndata), if(compoundgamma){list(varRrate=nrvars)}) )
}else{
